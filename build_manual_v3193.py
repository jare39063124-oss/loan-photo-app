...
def add_page_num(slide, num, total=14):
    """添加页码"""
...
add_text(slide, wx + Inches(0.3), wy + Inches(5.2), ww - Inches(0.6), Inches(0.35),
         "抚顺银行风险管理部", font_size=9, color=C_TEXT_DIM, align=PP_ALIGN.CENTER)

add_page_num(slide, 4)

# ============================================================
# Slide 5: 第三章 Excel格式规范
# ============================================================
slide = add_slide(prs)
...
tbl_x = Inches(0.4)
tbl_y = Inches(3.5)
col_widths = [Inches(2.0), Inches(3.5), Inches(2.5), Inches(2.0), Inches(2.4)]
headers = ["客户名", "地址概(街道+楼栋)", "地址精确(单元房号)", "性质", "备注"]
# 表头
cx = tbl_x
for i, (hdr, cw) in enumerate(zip(headers, col_widths)):
    cell = add_rect(slide, cx, tbl_y, cw, Inches(0.45), C_ACCENT)
    add_text(slide, cx, tbl_y, cw, Inches(0.45), hdr,
             font_size=11, bold=True, color=C_WHITE, align=PP_ALIGN.CENTER, anchor=MSO_ANCHOR.MIDDLE)
    cx += cw
# 数据行
data = [
    ["成都投资集团", "和平区XX街123号", "1-23-4", "抵押", ""],
    ["沈阳置业有限公司", "沈河区YY路45号", "2-15-1", "商铺", ""],
    ["抚顺实业公司", "新抚区ZZ道67号", "3-8-2", "抵押", "已拍照"],
    ["大连贸易有限公司", "中山区AA路89号", "5-3-7", "抵债", ""],
]
for r, row in enumerate(data):
    ...
    for i, (val, cw) in enumerate(zip(row, col_widths)):
        ...
        add_text(slide, cx + Inches(0.05), ry, cw - Inches(0.1), Inches(0.5), val,
                 font_size=10, color=C_TEXT, align=PP_ALIGN.CENTER, anchor=MSO_ANCHOR.MIDDLE)
        cx += cw
...
add_text(slide, sx + Inches(2.3), sy + Inches(3.62), Inches(3.0), Inches(0.3),
         "https://api.openai.com/v1", font_size=7, color=C_TEXT_DIM, anchor=MSO_ANCHOR.MIDDLE)
# 保存按钮
...
# ============================================================
# Slide 14: 第十二章 常见问题 + 末页
# ============================================================
slide = add_slide(prs)
add_title_bar(slide, 12, "常见问题")
