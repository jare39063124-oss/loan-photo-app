...
> **最新 APK**: `C:\Users\Administrator\Desktop\loan-photo-tool-v3.22.15.apk` (待构建)
> **最新 PPT**: `C:\Users\Administrator\Desktop\资产盘点拍照工具-使用说明书-v3.22.15.pptx` (待构建)
...
| **v3.22.15** | **P0:PhotoViewerPopup 可见性终极修复 — ThemedPopup 增加 background='' 禁用 Kivy Popup 默认 atlas 背景图像（某些机型加载失败导致 popup 完全不可见）+ _on_view_photos 用 Clock.schedule_once 延迟 0.1s 创建+open 避免触摸事件链时序冲突 + 防抖 500ms→1500ms 防重复点击 + popup 状态诊断日志（pos/size/opacity/parent）+ VisitNoteDialog 提示文案同步更新"同楼盘多房间合并说明"** | 待构建 |
