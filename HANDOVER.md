...
> **Git Commit**: `待更新`
> **GitHub Actions Run**: `待更新` (待构建)
> **最新 APK**: `C:\Users\Administrator\Desktop\loan-photo-tool-v3.22.19.apk` (待构建)
> **最新 PPT**: `C:\Users\Administrator\Desktop\资产盘点拍照工具-使用说明书-v3.22.19.pptx` (待生成, 16 页)
>
> **APK 下载链接(nightly.link)**:
> https://nightly.link/jare39063124-oss/loan-photo-app/actions/runs/28723802219/loan-photo-apk.zip
...
| **v3.22.19** | **综合增强：1) 缩略图方案替代原图查看（拍照后异步生成压缩缩略图到 app 私有目录，绕过 Scoped Storage 权限问题）+ 2) 键盘遮挡修复（softinput_mode 改 resize + 弹窗聚焦滚动）+ Spinner 倒角 + 3) AI 报告提示词调整（每客户仅 1 行 + 多地址分号成行 + 同地址 4+ 房间号合并 + few-shot 示例）+ 4) 默认文件名改为"线路四XX（日期）现场勘查日报表" + 5) 表头锁定 + 全选/行选择/反选（每行 CheckBox，支持全选后单行反选）+ 6) 最近列表数据指示器（绿/灰圆点）+ 2 次确认清除按钮** | 🔄 待构建 |
...
| 文件 | 说明 |
|------|------|
| `C:\Users\Administrator\Desktop\loan-photo-tool-v3.22.11.apk` | **最新 APK**(v3.22.11, ~29 MB) |
| `C:\Users\Administrator\Desktop\资产盘点拍照工具-使用说明书-v3.22.11.pptx` | **最新 PPT 说明书**(16 页) |
