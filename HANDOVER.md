...
> **Git Commit**: `b0ea7b5` (本地 `f7fc71f`，经 GitHub API 推送为 `b0ea7b5`，内容一致)
> **GitHub Actions Run**: `28868095783` (Build #97)
> **最新 APK**: `C:\Users\Administrator\Desktop\loan-photo-tool-v3.22.24.apk` (已构建, 29.05 MB)
> **最新 PPT**: `C:\Users\Administrator\Desktop\资产盘点拍照工具-使用说明书-v3.22.24.pptx` (已生成, 16 页, 68764 字节)
>
> **APK 下载链接(nightly.link)**:
> https://nightly.link/jare39063124-oss/loan-photo-app/actions/runs/28868095783/loan-photo-apk.zip
...
| **v3.22.23** | **回归修复+强化：1) 全选作用域再次修正（v3.22.22 用 self.row_widgets 仅选可见行是回归，改为 self._matched_indices 选中当前搜索条件下全部匹配行，如搜索"阳光路1号楼"匹配100条则全选100条而非仅渲染的5条）+ 2) 房间号2位分组排序算法（从右起按2位分组，末组为组2、其余为组1，按组1主→组2次升序，应用于AI报表_format_address_grouping和_collect_records）+ 3) PPT风格回退至v3.22.21（恢复使用build_manual_v3193.py纯矢量图形风格，停用make_ppt.py）+ 4) 最近文件数据计数隔离（新增excel_data_index.json映射表按Excel URI md5隔离progress_keys，不再跨文件串扰）+ 5) AI日报表强化：VisitNoteDialog新增"保存"按钮（仅持久化不生成报表）+REPORT_SYSTEM_PROMPT重构为5段分层优先级规则（删除冗长弘马大街示例避免过拟合）+_build_prompt显式标注"必须参考"补充说明与备注+_validate_ai_response校验AI是否参考visit_note+低质量报告提示用户人工核对 + 6) 多机型适配与bug审计（未发现严重问题）+ 7) TRAE AI创造力大赛参赛材料整理（trae-competition-entry.md，学习工作赛道，报名截止2026-07-15）** | ✅ 已构建 commit d08b999, Run 28790647079 |
| **v3.22.24** | **P0:相机闪退修复(华为Mate70/OPPO/小米13Pro)— camera_session.json状态持久化+进程重启恢复+on_camera_result异步化+移除Intent.createChooser+largeHeap + 500+全选拍照性能优化(mark_photo_batch单次save+_row_widget_map O(1)查找+_refresh_rows_done_batch批量刷新,消除3个O(N²)) + 列表滚动优化(缩略图LRU缓存256张) + 全局bug审计(5处裸except+1处_row_widget_map同步缺失)** | ✅ 已构建 commit b0ea7b5, Run 28868095783 |
